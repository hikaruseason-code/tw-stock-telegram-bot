import { isStockIdValid, fetchStockNews } from '../lib/stock.js'
import { getNewsListHTMLTemplate } from '../lib/template.js'

export async function handleStockNews(msg, match) {
  const chatId = msg.chat.id
  const stockId = match[1]
  if (!stockId) {
    return this.sendMessage(chatId, '請帶入股號\ne.g. `/news 2330`', { parse_mode: 'Markdown' })
  }
  ...
  this.sendMessage(chatId, HTML, { parse_mode: 'HTML', disable_web_page_preview: true })
}
